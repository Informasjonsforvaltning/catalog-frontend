...
export const hasResourceRole = (
  token: Token,
  { resource, resourceId, role }: ResourceRole,
): boolean =>
  !!getResourceRoles(token).find(
    (r) =>
      r.resource === resource && r.resourceId === resourceId && r.role === role,
  );

export const hasOrganizationRole = (
  token: Token,
  { orgNr, role }: OrganizationRole,
): boolean =>
  hasResourceRole(token, { resource: "organization", resourceId: orgNr, role });

export const hasOrganizationReadPermission = (
  token: Token,
  orgNr: string,
): boolean => {
...
export const hasOrganizationWritePermission = (
  token: Token,
  orgNr: string,
): boolean =>
...
export const hasNonSystemAccessForOrg = (
  token: Token,
  orgId: string,
): boolean => {
  return (
...
export const hasAcceptedTermsForOrg = (
  token: Token,
  orgId: string,
): boolean => {
...
export const hasAccessToMoreThanOneOrg = (token: Token): boolean => {
  return (
    new Set(getResourceRoles(token).map(({ resourceId }) => resourceId)).size >
    1
  );
};
