...
export default class DatasetEditPage {
  ...
  async fillDescriptionField(
    ...
    await this.fillLanguageField(
      value,
      "Beskrivelse Hjelp til utfylling Må fylles ut",
      open,
      clear,
    );
  }
  ...
  async selectAccessRights(
    ...
    const radio = accessRightsGroup.getByLabel(options[option], {
      exact: true,
    });
  ...
  async expectHelpButtons() {
    const helpButtons = this.page.getByRole("button", {
      name: "Hjelp til utfylling",
    });
  ...
  async clickAddDistribution() {
    await this.page
      .getByRole("button", { name: "Legg til distribusjon" })
      .click();
  }
  ...
  async addLandingPage(url: string, clickAddButton: boolean = true) {
    if (clickAddButton) {
      await this.page
        .getByRole("button", { name: "Legg til landingsside" })
        .click();
    }
  ...
  async selectProvenance(provenance: string) {
    const provenanceField = this.page.getByRole("combobox", {
      name: "Proveniens",
    });
  ...
  async selectFrequency(frequency: string) {
    const frequencyField = this.page.getByRole("combobox", {
      name: "Frekvens",
    });
  ...
  async fillRelationForm(data: { relationType: string; dataset: string }) {
    const dialog = this.page.getByRole("dialog");
    await dialog
      .getByRole("group", { name: "Relasjonstype" })
      .getByRole("combobox")
      .click();
  ...
  async clickAddContactPoint() {
    await this.page
      .getByRole("button", { name: "Legg til kontaktpunkt" })
      .click();
  }
