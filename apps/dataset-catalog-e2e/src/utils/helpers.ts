...
export const createDataset = async (apiRequestContext, dataset) => {
  ...
  if (!response.ok()) {
    console.error(
      `[CREATE DATASET] API call failed with status ${response.status()}`,
      await response.json(),
    );
    throw new Error(`API call failed with status ${response.status()}`);
  }
  ...
  console.log(
    "[CREATE DATASET] Dataset created successfully with ID:",
    createdDataset,
  );
...
export const getPublishedDataset = async (apiRequestContext) => {
  ...
  if (!response.ok()) {
    console.error(
      "[GET PUBLISHED DATASET] API call failed with status",
      response.status(),
    );
    throw new Error(`API call failed with status ${response.status()}`);
  }
...
export const publishDataset = async (apiRequestContext, datasetId) => {
  ...
  if (!response.ok()) {
    console.error(
      `[PUBLISH DATASET] API call failed with status ${response.status()}`,
      await response.json(),
    );
    throw new Error(`API call failed with status ${response.status()}`);
  }
