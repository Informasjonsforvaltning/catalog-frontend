...
runTestAsAdmin('should show restore dialog after refresh when form has unsaved changes', async ({ page, datasetsPage, playwright }) => {
...
runTestAsAdmin('should restore form data when clicking restore button', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  const newTitle = {
    nb: uniqueString('new_title_nb'),
    nn: uniqueString('new_title_nn'),
...
runTestAsAdmin('should not show restore dialog after clicking discard button', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  const newTitle = {
    nb: uniqueString('new_title_nb'),
    nn: uniqueString('new_title_nn'),
...
runTestAsAdmin('should show restore dialog multiple times until data is discarded', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  const newTitle = {
    nb: uniqueString('new_title_nb'),
    nn: uniqueString('new_title_nn'),
...
runTestAsAdmin('should auto-save modal dialog data and show restore dialog', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  ...
  await editPage.fillUrlWithLabelModal('Leg til skjermingshjemmel', modalData, ['Bokmål', 'Nynorsk', 'Engelsk'], false);
  await editPage.waitForAutoSaveToComplete();
...
runTestAsAdmin('should auto-save distribution modal data and restore correctly', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  ...
  await expect(page.getByText(distributionData.description.nb)).toBeVisible();
});

runTestAsAdmin('should not show restore dialog when form has no unsaved changes', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
...
runTestAsAdmin('should clear auto-save data after successful form submission', async ({ page, datasetsPage, playwright }) => {
  const dataset = await createRandomDataset(playwright);
  const newTitle = {
    nb: uniqueString('new_title_nb'),
    nn: uniqueString('new_title_nn'),
