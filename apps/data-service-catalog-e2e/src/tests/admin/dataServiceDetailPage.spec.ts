...
runTestAsAdmin(
  ...
  async ({ page, accessibilityBuilder, playwright }) => {
    ...
    await dataServiceDetailPage.goto(
      process.env.E2E_CATALOG_ID as any,
    ...
    await dataServiceDetailPage.expectDescriptionToBe(
      dataService.description.nb as string,
    );
    await dataServiceDetailPage.expectEndpointUrlToBe(
...
runTestAsAdmin(
  "should publish data service with confirmation",
  async ({ page, playwright, accessibilityBuilder }) => {
    ...
    await expect(page.locator("dialog[open]")).toContainText(
      "Er du sikker på at du vil publisere API-beskrivelsen?",
    );
...
runTestAsAdmin(
  ...
  async ({ page, playwright, accessibilityBuilder }) => {
    ...
    await dataServiceDetailPage.expectConfirmModalVisible();
    await expect(page.locator("dialog[open]")).toContainText(
      "Er du sikker på at du vil avpublisere API-beskrivelsen?",
    );
...
runTestAsAdmin(
  "should cancel publish confirmation modal",
  async ({ page, playwright, accessibilityBuilder }) => {
    const apiRequestContext = await playwright.request.newContext({
      storageState: adminAuthFile,
    });
...
runTestAsAdmin(
  "should delete data service with confirmation",
  async ({ page, playwright, accessibilityBuilder }) => {
    const apiRequestContext = await playwright.request.newContext({
      storageState: adminAuthFile,
    });
    ...
    await dataServiceDetailPage.expectConfirmModalVisible();
    await expect(page.locator("dialog[open]")).toContainText(
      "Er du sikker på at du vil slette API-beskrivelsen?",
    );
...
runTestAsAdmin(
  "should cancel delete confirmation modal",
  async ({ page, playwright, accessibilityBuilder }) => {
    const apiRequestContext = await playwright.request.newContext({
      storageState: adminAuthFile,
    });
...
runTestAsAdmin(
  "should show correct URL structure",
  async ({ page, playwright, accessibilityBuilder }) => {
    const apiRequestContext = await playwright.request.newContext({
      storageState: adminAuthFile,
    });
