...
export const adminAuthFile = path.join(process.cwd(), 'apps/data-service-catalog-e2e/.playwright/auth/admin.json');
export const writeAuthFile = path.join(process.cwd(), 'apps/data-service-catalog-e2e/.playwright/auth/write.json');
export const readAuthFile = path.join(process.cwd(), 'apps/data-service-catalog-e2e/.playwright/auth/read.json');
...
export const deleteAllDataServices = async (apiRequestContext: any) => {
  console.log('[DELETE ALL DATA SERVICES] Starting deletion of all data services...');

  try {
    const response = await apiRequestContext.get(`/api/catalogs/${process.env.E2E_CATALOG_ID}/data-services`);
    if (!response.ok()) {
    ...
    console.log(`[DELETE ALL DATA SERVICES] Found ${dataServices.length} data services`);
    for (const dataService of dataServices) {
      if (!dataService.published) {
        console.log(`[DELETE ALL DATA SERVICES] Deleting data service: ${dataService.id}`);
        await apiRequestContext.delete(`/api/catalogs/${process.env.E2E_CATALOG_ID}/data-services/${dataService.id}`);
      }
    }

    console.log('[DELETE ALL DATA SERVICES] All data services deleted successfully');
  } catch (error) {
    console.error('[DELETE ALL DATA SERVICES] Error deleting data services:', error);
    throw error;
  }
};

export const deleteDataService = async (apiRequestContext: any, dataServiceId: any) => {
  console.log('[DELETE DATA SERVICE] Deleting data service:', dataServiceId);
  await apiRequestContext.delete(`/api/catalogs/${process.env.E2E_CATALOG_ID}/data-services/${dataServiceId}`);
  console.log('[DELETE DATA SERVICE] Data service deleted successfully');
};

export const createDataService = async (apiRequestContext: any, dataService: any) => {
  console.log('[CREATE DATA SERVICE] Creating data service:', dataService.title);
  const response = await apiRequestContext.post(`/api/catalogs/${process.env.E2E_CATALOG_ID}/data-services`, {
    data: dataService,
  });

  if (!response.ok()) {
    console.error(`[CREATE DATA SERVICE] API call failed with status ${response.status()}`, await response.json());
    throw new Error(`API call failed with status ${response.status()}`);
  }

  const createdDataService = await response.json();
  console.log('[CREATE DATA SERVICE] Data service created successfully with ID:', createdDataService.id);
...
export const getPublishedDataService = async (apiRequestContext: any) => {
  ...
  const response = await apiRequestContext.get(`/api/catalogs/${process.env.E2E_CATALOG_ID}/data-services`);
  if (!response.ok()) {
    console.error('[GET PUBLISHED DATA SERVICE] API call failed with status', response.status());
    throw new Error(`API call failed with status ${response.status()}`);
  }

  const dataServices = await response.json();
  const publishedDataService = dataServices.filter((d: any) => d.published).pop();
  console.log('[GET PUBLISHED DATA SERVICE] Found published data service:', publishedDataService?.id);
  return publishedDataService;
};

export const publishDataService = async (apiRequestContext: any, dataServiceId: any) => {
  ...
  if (!response.ok()) {
    console.error(`[PUBLISH DATA SERVICE] API call failed with status ${response.status()}`, await response.json());
    throw new Error(`API call failed with status ${response.status()}`);
  }
