...
runTestAsAdmin('should show restore dialog after refresh when concept form has unsaved changes', async ({ page, conceptsPage, playwright }) => {
...
runTestAsAdmin('should restore concept form data when clicking restore button', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
  const newTerm = {
    nb: uniqueString('new_term_nb'),
    nn: uniqueString('new_term_nn'),
...
runTestAsAdmin('should not show restore dialog after clicking discard button', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
  const newTerm = {
    nb: uniqueString('new_term_nb'),
    nn: uniqueString('new_term_nn'),
...
runTestAsAdmin('should show restore dialog multiple times until data is discarded', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
  const newTerm = {
    nb: uniqueString('new_term_nb'),
    nn: uniqueString('new_term_nn'),
...
runTestAsAdmin('should auto-save relation modal data and show restore dialog', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
...
runTestAsAdmin('should not show restore dialog when concept form has no unsaved changes', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
...
runTestAsAdmin('should clear auto-save data after successful concept form submission', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
  const newTerm = {
    nb: uniqueString('new_term_nb'),
    nn: uniqueString('new_term_nn'),
...
runTestAsAdmin('should auto-save tillatt term data and restore correctly', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
...
runTestAsAdmin('should discard auto-save data when changes are reverted to original state', async ({ page, conceptsPage, playwright }) => {
  const concept = await createRandomConcept(playwright);
  const newTerm = {
    nb: uniqueString('new_term_nb'),
    nn: uniqueString('new_term_nn'),
