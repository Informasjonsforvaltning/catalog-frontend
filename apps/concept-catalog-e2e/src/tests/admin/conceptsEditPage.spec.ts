...
runTestAsAdmin(
  "test if updating an existing concept saves correctly",
  async ({ conceptsPage, playwright }) => {
    console.log(
      "[TEST] Starting test: test if updating an existing concept saves correctly",
    );
    ...
    await conceptsPage.editPage.fillFormAndSave(
      updatedConcept,
      apiRequestContext,
      true,
    );
    ...
    await conceptsPage.detailPage.expectDetails(
      updatedConcept,
      apiRequestContext,
    );

    console.log(
      "[TEST] Test completed: test if updating an existing concept saves correctly",
    );
  },
);
...
runTestAsAdmin(
  ...
  async ({ conceptsPage, playwright }) => {
    console.log(
      "[TEST] Starting test: test that it should not be possible to make a relation to itself",
    );
...
runTestAsAdmin(
  "test that URL fields only accept HTTPS URLs",
  async ({ conceptsPage, playwright }) => {
    console.log(
      "[TEST] Starting test: test that URL fields only accept HTTPS URLs",
    );
    ...
    console.log(
      "[TEST] Testing definition source URI with HTTP URL in modal...",
    );
    await conceptsPage.page
    ...
    await conceptsPage.page
      .getByRole("textbox", { name: "https://kilde.no" })
      .fill("http://invalid-source.com");
    await conceptsPage.page
    ...
    await conceptsPage.page
      .getByRole("textbox", { name: "https://kilde.no" })
      .fill("https://valid-https-source.com");
    await conceptsPage.page
      .getByRole("button", { name: "Legg til definisjon" })
      .click();
