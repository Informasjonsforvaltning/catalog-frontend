...
const hasNoFieldValues = (values: DataServiceCost) => {
  ...
  return (
    _.isEmpty(values.value) &&
    _.isEmpty(values.description) &&
    _.isEmpty(values.documentation) &&
    _.isEmpty(values.currency)
  );
};
...
export const CostsTable = ({ currencies }: Props) => {
  ...
  return (
    <div className={styles.fieldContainer}>
      ...
      <div>
        <FieldModal
          template={{ currency: DEFAULT_CURRENCY }}
          type={'new'}
          currencies={sortedCurrencies}
...
const FieldModal = ({ template, type, onSuccess, currencies }: ModalProps) => {
  ...
  const rmCurrencyIfNoValue = (formValues: DataServiceCost) => {
    if (_.isNumber(formValues.value)) {
      return formValues;
    }
                  ...
                  >
                    <div className={styles.valueCurrencyFieldset}>
                      ...
                      <Combobox
                        ...
                        size='sm'
                        disabled={!_.isNumber(values?.value)}
                      >
                        {currencies &&
