...
export async function POST(
  ...
  try {
    ...
    const response = await publishDataService(catalogId, dataServiceId, `${session.accessToken}`);
    if (!response.ok) {
    ...
    return NextResponse.json({ error: 'Failed to publish data service' }, { status: 500 });
  }
}
