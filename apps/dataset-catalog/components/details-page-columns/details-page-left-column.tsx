...
export const LeftColumn = ({
  ...
  const { data: qualifiedAttributions } = useSearchEnheterByOrgNmbs(
  ...
  const { data: references } = useSearchDatasetsByUri(
    searchEnv,
    allReferenceUris,
  );
  ...
  return (
    <InfoCard>
      ...
      {(hasValues(dataset?.accessRight) ||
        ...
        <InfoCard.Item title={localization.access} className={styles.access}>
            ...
            <TagList
              ...
              getTagText={(item) => {
                const match =
                  ...
                  referenceData.dataThemes.find((theme) => theme?.uri === item);
      ...
      {hasValues(dataset?.distribution) && (
        <InfoCard.Item
          title={localization.datasetForm.fieldLabel.distributions}
        >
      ...
      {hasValues(dataset?.qualifiedAttributions) && (
        <InfoCard.Item
          title={localization.datasetForm.fieldLabel.qualifiedAttributions}
        >
          <TagList
      ...
      {(hasValues(dataset.references) ||
        hasValues(dataset.relatedResources)) && (
          ...
          <div className={styles.infoCardItems}>
            ...
            {hasValues(dataset?.inSeries) && (
              <div>
                ...
                {(() => {
                  const matchedType = datasetSeries.find(
        ...
        >
          <Table size="sm">
            <Table.Head>
              <Table.Row>
                <Table.HeaderCell>
                  {localization.resourceType.informationModel}
                </Table.HeaderCell>
                <Table.HeaderCell>{localization.publisher}</Table.HeaderCell>
              </Table.Row>
            </Table.Head>
            <Table.Body>
              {dataset?.informationModelsFromFDK &&
                dataset?.informationModelsFromFDK.map((item, index) => {
                  const match = informationModels?.find(
                    (model) => model?.uri === item,
                  );
                  return match ? (
