...
import { adminAuthFile, createService, publishService, ServiceStatus, uniqueString } from '../../utils/helpers';
...
runTestAsAdmin('test search', async ({ servicesPage, playwright }) => {
  ...
  const randomServices: Partial<Service>[] = Array.from({ length: 3 }).map((_, i) => {
    ...
    const result1aDescNb = uniqueString(`This_is_service_result_${i + 1}a_produced_by_service_${i + 1}_nb`);
    const result1aDescNn = uniqueString(`This_is_service_result_${i + 1}a_produced_by_service_${i + 1}_nn`);
    const result1aDescEn = uniqueString(`This_is_service_result_${i + 1}a_produced_by_service_${i + 1}_en`);
    ...
    const result1bDescNb = uniqueString(`This_is_service_result_${i + 1}b_produced_by_service_${i + 1}_nb`);
    const result1bDescNn = uniqueString(`This_is_service_result_${i + 1}b_produced_by_service_${i + 1}_nn`);
    const result1bDescEn = uniqueString(`This_is_service_result_${i + 1}b_produced_by_service_${i + 1}_en`);
    const contactCategoryNb = uniqueString(`Category_contact_point_service_${i + 1}_nb`);
    const contactCategoryNn = uniqueString(`Category_contact_point_service_${i + 1}_nn`);
    const contactCategoryEn = uniqueString(`Category_contact_point_service_${i + 1}_en`);
...
runTestAsAdmin('test published state filter', async ({ servicesPage, playwright }) => {
  const apiRequestContext = await playwright.request.newContext({
    storageState: adminAuthFile,
  });
