...
export const deleteAllServices = async (apiRequestContext: any) => {
  ...
  for (const service of services) {
    await apiRequestContext.delete(`/api/catalogs/${process.env.E2E_CATALOG_ID}/services/${service.id}`);
  }
};
...
export const deleteAllPublicServices = async (apiRequestContext: any) => {
  ...
  const services = await response.json();
  for (const service of services) {
    await apiRequestContext.delete(`/api/catalogs/${process.env.E2E_CATALOG_ID}/public-services/${service.id}`);
  }
};
