...
import { toMutableInformationModelValues } from "@information-model-catalog/utils/information-model";
...
export async function createInformationModel(
  ...
  const newInformationModel = toMutableInformationModelValues(values);
  const session = await getValidSession();
  if (!session) {
    return redirectToSignIn();
  }
...
export async function updateInformationModel(
  ...
  const diff = compare(
    toMutableInformationModelValues(initialInformationModel),
    toMutableInformationModelValues(values),
  );
  ...
  try {
    ...
    if (response.status !== 200) {
      ...
      console.error(
        `Failed to update information model: ${response.status} ${response.statusText}`,
        errorBody,
      );
